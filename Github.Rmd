...
```{r}
plot21 <- read_delim("Z2_16S_Tax_summ.csv", ",", escape_double = FALSE, col_names = TRUE, trim_ws = TRUE)
library(ggplot2)
plot22 <- ggplot(plot21, aes(x = SampleID, y = Tax, fill = Counts,)) + geom_tile() + scale_fill_distiller(palette = 'YlGnBu')
plot22+ scale_fill_distiller(palette = 'blues')
plot22 scale_fill_gradient()
plot22
#save plot to working directory
ggsave(plot3,filename="z21.png",height=9,width=12,units="in",dpi=200)
```

```{r}
#Los 10 mejores reads
library(ggplot2)
...
```{r}
library(ggplot2)
plot_10z <- read_delim("allZ.csv", ",", escape_double = FALSE, col_names = TRUE, trim_ws = TRUE)
plot_best <- ggplot(plot_10z, aes(x = SampleID, y = Counts, fill = Tax,)) + geom_bar(aes(), stat="identity", position="stack")
plot_best

plot_12 <- read_delim("allZ.csv", ",", escape_double = FALSE, col_names = TRUE, trim_ws = TRUE)
library(ggplot2)
